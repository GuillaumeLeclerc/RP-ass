...
trait SwingApi {
    ...
    def textValues: Observable[String] = {
      Observable.apply(obs => {
    	  val myReaction : Reaction= {
	        case ValueChanged(x) => obs.onNext(x.text)
	        case _ =>
	      }
    	  field.subscribe(myReaction)
    	  Subscription{
    	    field.unsubscribe(myReaction)
    	  }
      })
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      Observable.apply(obs => {
        val myReaction : Reaction = {
          case ButtonClicked(x) => obs.onNext(x)
          case _ =>
        }
        button.subscribe(myReaction)
        Subscription{
          button.unsubscribe(myReaction)
        }
      })
    }
